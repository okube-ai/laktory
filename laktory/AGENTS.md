...
- schema_name: yahoo
  ...
  mode: APPEND  # required for a streaming source - no implicit default
- schema_name: yahoo
  table_name: slv_stock_prices_quarantine
  is_quarantine: true  # mode defaults to APPEND for is_quarantine sinks on a streaming source
transformer:
  nodes:
  - expr: |
      SELECT
...
- Any Laktory setting except `workspace_root` (`build_root`, `dataframe_api`, `dataframe_backend`, `runtime_root`, `cli_raise_external_exceptions`, `register_nw_extensions`, `log_level`) can be set via a `laktory_settings_<field>` bundle variable in `databricks.yml` (e.g. `laktory_settings_runtime_root: /Volumes/<catalog>/<schema>/<volume>/<path>` to replace what `settings.runtime_root` did stack-wide under Terraform — needed when the default checkpoint location fails with `[DBFS_DISABLED]`), or via the matching env var (e.g. `LAKTORY_RUNTIME_ROOT`). `workspace_root` is not settable this way — it keeps its own dedicated `dab_workspace_root`-based auto-configuration (see above), which is resolved *after* any `build_root` override so it never derives from a stale value
