...
- Any other Laktory setting (`dataframe_api`, `dataframe_backend`, `runtime_root`, `cli_raise_external_exceptions`, `register_nw_extensions`, `log_level`) can be set once for the whole deployment via a `laktory_settings_<field>` bundle variable (e.g. `laktory_settings_runtime_root: /Volumes/.../checkpoints/`), instead of an env var or repeating a per-pipeline field on every pipeline. A per-pipeline field still takes priority when both are set. `workspace_root`/`build_root` are not settable this way — they keep their own dedicated auto-configuration (see above)
