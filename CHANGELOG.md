...
* Fixed a sink with no explicit `mode:` crashing with `Mode 'None' is not supported for Spark Streaming DataFrame` when it's an `is_quarantine: true` sink on a streaming node - it now defaults to `APPEND`. [[#661](https://github.com/okube-ai/laktory/issues/661)]
### Updated
