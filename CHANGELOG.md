...
* Documented that `mode` has no implicit default for any sink - including `is_quarantine: true` ones, which need `mode: APPEND` set explicitly like any other sink - and fixed `docs/concepts/dataquality.md` and `laktory/AGENTS.md`'s own quarantine-sink examples, which omitted `mode:` entirely. [[#661](https://github.com/okube-ai/laktory/issues/661)]
### Breaking changes
* n/a
