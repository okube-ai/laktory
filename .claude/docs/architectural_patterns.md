...
## 17. Rendered File Content Pattern

Some resources point at a local file whose *content* (not just field values) is uploaded verbatim by Terraform — `Dashboard.file_path`, `WorkspaceFile.source`, `Notebook.source`, `DbfsFile.source`, and files generated by `WorkspaceTree`. Variable injection (section 7) only touches field values, so by default this content is never `${vars.x}`-resolved.

The reusable pattern for opting a resource's file content into rendering — do **not** reach for `content_base64`/`serialized_*` inline embedding for this (discouraged, bloats Terraform state):

1. Give the field a `xxx_`/computed-`xxx` alias override (`validation_alias=AliasChoices("xxx", "xxx_")`, `exclude=True`, plus `@computed_field def xxx`) so the public field can be transparently redirected. Generated `*_base.py` classes must have the plain field added to `PER_RESOURCE_SKIP_ATTRS` in `scripts/build_resources/01_build.py` first (see section 11), or the computed-field override conflicts with the generated plain field.
2. Mix in `RenderableFileMixin` (`laktory/models/resources/databricks/_renderablefile.py`) for the shared `render_vars` opt-in field and `_staged_path`/`_render_to_staged_path` helpers.
3. The computed field returns a path under `settings.build_root` when `render_vars` is `True`, else the original path unchanged — never based on the field itself (avoid a `xxx → path → filename → xxx` cycle; derive `filename`/staging keys from the raw `xxx_`, not the computed `xxx`).
4. A `build(self, vars=None)` method reads the original file, resolves it via `self.resolve_string(text, vars=vars)` (thin wrapper over `inject_vars_into_dump`, in `basemodel.py`), and writes the result to the staged path.
5. Add `"render_vars"` to the class's `terraform_excludes` — it's a Laktory-only flag, not a real Terraform argument.
6. Wire `.build()` calls into `Stack.build()` (`laktory/models/stacks/stack.py`) so rendering runs automatically before every `preview`/`deploy`/`destroy` (which all call `env.build()` via `to_terraform()`), not only on an explicit `laktory build`.

First established by `PipelineConfigWorkspaceFile` (pipeline config JSON, step 1 pre-existing via `WorkspaceFile`), generalized to `WorkspaceFile`, `Notebook`, `DbfsFile`, and `Dashboard` via `RenderableFileMixin`. `WorkspaceTree` exposes bulk opt-in via `render_paths` (glob patterns relative to `source`, matched with `PurePosixPath.match()` - a bare `*.ext` pattern matches at any depth, so it doubles as extension-based opt-in without a separate field) and sets `render_vars` on the `WorkspaceFile`/`Notebook` instances it generates per file.
