...
def emit_resource_module(resource_key: str, resource_schema: dict) -> str:
    ...
    header = textwrap.dedent(f"""\
        ...
        # Regenerate with: python scripts/poc_tf_schema_gen/approach_a_templates.py {resource_key}
        from __future__ import annotations
...
def main():
    ...
    for resource_key in targets:
        ...
        class_name = resource_to_class_name(resource_key)
        out_path = OUTPUT_DIR / f"{class_name.lower()}_gen_a.py"
        out_path.write_text(code)
        print(f"[OK]   {resource_key} -> {out_path.relative_to(Path(__file__).parent)}")
